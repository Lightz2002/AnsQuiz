...
class App {
    ...
    constructor() {
        ...
        paginationContainer.addEventListener(
            "click",
            this._switchQuestion.bind(this, this.#questionLists, "question-list-item", "")
        );
        this.#btnContainer.addEventListener(
            "click",
            this._switchQuestion.bind(this, this.#questionLists, "btn-next", "btn-prev")
        );
    ...
    _checkActiveAnswer() {
        return [...this.#choicesContainer.children].find((e) => e.classList.contains("chosen"));
    }
    ...
    _renderChoices(questionObj) {
        const choices = questionObj.choices.map(
            (choice, index) => `
          <div class="btn btn-choices ${questionObj.choosedAnswer === choice ? "chosen" : ""}">
              <input
                  class="input-radio"
                  id="${choice}"
                  type="radio"
    ...
    _renderQuestionPagination(questionLists) {
        ...
        questionLists.forEach((question) => {
            ...
            console.log(this.#currentPage, questionNumber);
            const html = `
    ...
    _switchQuestion(questionLists, classes1, classes2, e) {
        ...
        if (target.classList.contains(classes1) || target.classList.contains(classes2)) {
    ...
    _toggleActiveAnswer(questionLists, e) {
        ...
        if (activeChoices) {
            ...
            this._renderQuestionPagination(questionLists);
            if (activeChoices == target || activeChoices == target.parentElement) return;
        }

        if (target.classList.contains("choices-label")) {
            target.parentElement.classList.add("chosen");
            questionLists[this.#currentPage].choosedAnswer = target.textContent.slice(4);
            this._renderQuestionPagination(questionLists);
            this._toggleSubmitButton(questionLists);
